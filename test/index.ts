...
test.before(async () => {
  ...
  switch (process.platform) {
    case 'linux':
      pathToBinary = path.join(__dirname, '../dist/linux-unpacked/webterm');
      break;

    case 'darwin':
      pathToBinary = path.join(__dirname, '../dist/mac/webterm.app/Contents/MacOS/webterm');
      break;

    case 'win32':
      pathToBinary = path.join(__dirname, '../dist/win-unpacked/webterm.exe');
      break;
