// Print diagnostic information for a few arguments instead of running webterm.
if (['--help', '-v', '--version'].includes(process.argv[1])) {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const {version} = require('./package');
  const configLocation = process.platform === 'win32' ? `${process.env.userprofile}\\.webterm.js` : '~/.webterm.js';
  console.log(`WebTerm version ${version}`);
  console.log('WebTerm does not accept any command line arguments. Please modify the config file instead.');
  console.log(`WebTerm configuration file located at: ${configLocation}`);
  process.exit();
}
    ...
    .then(() => {
      ...
      if (!isDev) {
        ...
        if (config.getConfig().defaultSSHApp && !app.isDefaultProtocolClient('ssh')) {
          console.log('Setting webterm as default client for ssh:// protocol');
          app.setAsDefaultProtocolClient('ssh');
        } else if (!config.getConfig().defaultSSHApp && app.isDefaultProtocolClient('ssh')) {
          console.log('Removing webterm from default client for ssh:// protocol');
