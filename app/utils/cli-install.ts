...
const addSymlink = async (silent: boolean) => {
  try {
    ...
    if (isInstalled) {
      console.log('webterm CLI already in PATH');
      return;
    }
    console.log('Linking webtermCLI');
    ...
    if (err.code === 'EACCES' && !silent) {
      const result = await dialog.showMessageBox({
        message: `You need to grant elevated privileges to add webterm CLI to PATH
Or you can run
sudo ln -sf "${cliScriptPath}" "${cliLinkPath}"`,
      ...
      if (result.response === 0) {
        try {
          await sudoExec(`ln -sf "${cliScriptPath}" "${cliLinkPath}"`, {name: 'webterm'});
          return;
        } catch (_error) {
...
const addBinToUserPath = () => {
  return new Promise<void>((resolve, reject) => {
    try {
      ...
      // C:\Users\<user>\AppData\Local\Programs\webterm\resources\bin
      const binPath = path.dirname(cliScriptPath);
      // C:\Users\<user>\AppData\Local\webterm
      const oldPath = path.resolve(process.env.LOCALAPPDATA!, 'webterm');
      ...
      console.log('Adding webtermCLI path (registry)');
...
export const installCLI = async (withNotification: boolean) => {
  if (process.platform === 'win32') {
    try {
      ...
      logNotify(
        withNotification,
        'webterm CLI installed',
        'You may need to restart your computer to complete this installation process.'
      );
    } catch (err) {
      logNotify(withNotification, 'webterm CLI installation failed', `Failed to add webterm CLI path to user PATH ${err}`);
    }
    ...
    try {
      ...
      logNotify(withNotification, 'webterm CLI installed', `Symlink created at ${cliLinkPath}`);
    } catch (error) {
      logNotify(withNotification, 'webterm CLI installation failed', `${error}`);
    }
  } else {
    logNotify(withNotification, 'webterm CLI installation failed', `Unsupported platform ${process.platform}`);
  }
};
