...
const buildFeedUrl = (canary: boolean, currentVersion: string) => {
  ...
  return `https://${updatePrefix}.webterm.is/update/${isLinux ? 'deb' : platform}${archSuffix}/${currentVersion}`;
};
...
export default (win: BrowserWindow) => {
  ...
  const onupdate = (
    ...
    const releaseUrl = updateUrl || `https://github.com/vercel/webterm/releases/tag/${releaseName}`;
