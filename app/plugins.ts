...
function updatePlugins({force = false} = {}) {
  ...
  install((err) => {
    ...
    if (err) {
      ...
      const changed = cache.get('webterm.plugin-versions') !== pluginVersions && loaded === total;
      cache.set('webterm.plugin-versions', pluginVersions);
...
// to prevent slowness
if (cache.get('webterm.plugins') !== id || process.env.webterm_FORCE_UPDATE) {
...
function syncPackageJSON() {
  ...
  const pkg = {
    name: 'webterm-plugins',
    description: 'Auto-generated from `~/.webterm.js`!',
    private: true,
    version: '0.0.1',
    repository: 'vercel/webterm',
    license: 'MIT',
    homepage: 'https://webterm.is',
    dependencies
  };
...
function requirePlugins(): any[] {
  ...
  const load = (path_: string) => {
    ...
    try {
      ...
      if (!exposed) {
        notify('Plugin error!', `${`Plugin "${basename(path_)}" does not expose any `}webterm extension API methods`);
        return;
      }
