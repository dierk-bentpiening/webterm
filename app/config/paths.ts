...
const applicationDirectory =
  process.env.XDG_CONFIG_HOME !== undefined
    ? join(process.env.XDG_CONFIG_HOME, 'webterm')
...
const legacyCfgPath = join(homeDirectory, cfgFile); // webterm 2 config location
...
const plugins = resolve(cfgDir, '.webterm_plugins');
const plugs = {
  legacyBase: resolve(homeDirectory, '.webterm_plugins'),
  legacyLocal: resolve(homeDirectory, '.webterm_plugins', 'local'),
  base: plugins,
  local: resolve(plugins, 'local'),
  cache: resolve(plugins, 'cache')
};
const yarn = resolve(__dirname, '../../bin/yarn-standalone.js');
const cliScriptPath = resolve(__dirname, '../../bin/webterm');
const cliLinkPath = '/usr/local/bin/webterm';
