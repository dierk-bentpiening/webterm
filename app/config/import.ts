...
// Migrate webterm2 config to webterm3 but only if the user hasn't manually
// touched the new config and if the old config is not a symlink
const migratewebterm2Config = () => {
  if (cfgPath === legacyCfgPath) {
    // No need to migrate
    return;
  }
  if (!existsSync(legacyCfgPath)) {
    // Already migrated or user never used webterm 2
    return;
  }
  ...
  if (existsNew) {
    ...
    if (hasNewConfigBeenTouched) {
      ...
      notify(
        'webterm 3',
        `Settings location has changed to ${cfgPath}.\nWe've backed up your old webterm config to ${backupPath}`
      );
      return;
    }
  }
