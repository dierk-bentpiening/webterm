...
const checkConfig = () => {
  ...
  msg += 'Please launch webterm and retry.';
  console.error(msg);
  process.exit(1);
};
...
const lsRemote = (pattern?: string) => {
  ...
  const URL = `https://api.npms.io/v2/search?q=${
    (pattern && `${pattern}+`) || ''
  }keywords:webterm-plugin,webterm-theme&size=250`;
    ...
    commandPromise = lsRemote(query)
      .then((entries) => {
        if (entries.length === 0) {
          ...
          console.error(`${chalk.red('Try')} ${chalk.green('webterm ls-remote')}`);
          process.exit(1);
        } else {
...
args.command('<default>', 'Launch webterm');
...
const main = (argv: string[]) => {
  ...
  const env = Object.assign({}, process.env, {
    // this will signal webterm that it was spawned from this module
    webterm_CLI: '1',
  ...
  if (!flags.verbose) {
    ...
    if (process.platform === 'darwin') {
      //Use `open` to prevent multiple webterm process
      const cmd = `open -b co.zeit.webterm ${args_}`;
      const opts = {
        env
      };
