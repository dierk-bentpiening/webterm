...
import {webtermState, webtermDispatch, webtermActions, ITermGroups} from '../hyper';
...
export function resetFontSize(): webtermActions {
  return {
...
export function windowGeometryUpdated({isMaximized}: {isMaximized: boolean}): webtermActions {
  return {
...
const getGroupUids = (state: webtermState) => {
...
export function enterFullScreen(): webtermActions {
  return {
    type: UI_ENTER_FULLSCREEN
  };
}

export function leaveFullScreen(): webtermActions {
  return {
...
export function execCommand(command: string, fn: (e: any, dispatch: webtermDispatch) => void, e: any) {
  return (dispatch: webtermDispatch) =>
    dispatch({
