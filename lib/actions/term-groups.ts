...
import {ITermState, ITermGroup, webtermState, webtermDispatch, webtermActions} from '../hyper';

function requestSplit(direction: 'VERTICAL' | 'HORIZONTAL') {
  return (activeUid: string) =>
    (dispatch: webtermDispatch, getState: () => webtermState): void => {
      dispatch({
...
export function resizeTermGroup(uid: string, sizes: number[]): webtermActions {
  return {
    uid,
