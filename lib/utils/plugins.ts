...
// https://github.com/vercel/webterm/issues/619
...
let connectors: {
  ...
  webterm: {state: any[]; dispatch: any[]};
...
Module._load = function _load(path: string) {
  ...
  switch (path) {
    ...
    case 'react-dom':
      ...
      return ReactDOM;
    case 'webterm/component':
      console.warn(
        'DEPRECATED: If your plugin requires `webterm/component`, it must requires `react.PureComponent` instead and bundle `react` as a dependency'
      );
      return PureComponent;
    case 'webterm/notify':
      return notify;
    case 'webterm/Notification':
      return Notification;
    case 'webterm/decorate':
      return decorate;
    case 'child_process':
...
const loadModules = () => {
  ...
  modules = paths.plugins
    ...
    .map((path) => {
      ...
      // mapwebtermTermState mapping for backwards compatibility with webtermterm
      if (mod.mapwebtermTermState) {
        mod.mapwebtermState = mod.mapwebtermTermState;
        console.error('mapwebtermTermState is deprecated. Use mapwebtermState instead.');
      }

      // mapwebtermTermDispatch mapping for backwards compatibility with webtermterm
      if (mod.mapwebtermTermDispatch) {
        mod.mapwebtermDispatch = mod.mapwebtermTermDispatch;
        console.error('mapwebtermTermDispatch is deprecated. Use mapwebtermDispatch instead.');
      }
      ...
      if (mod.mapwebtermState) {
        connectors.webterm.state.push(mod.mapwebtermState);
      }

      if (mod.mapwebtermDispatch) {
        connectors.webterm.dispatch.push(mod.mapwebtermDispatch);
      }
    ...
    .filter((mod): mod is webtermPlugin => Boolean(mod));
...
export function connect<stateProps, dispatchProps>(
  stateFn: (state: webtermState) => stateProps,
  dispatchFn: (dispatch: webtermDispatch) => dispatchProps,
  c: any,
  d: Options = {}
) {
  return (Class: any, name: keyof typeof connectors) => {
    return reduxConnect<stateProps, dispatchProps, any, webtermState>(
      (state) => {
